    ...
    public class ContractQualityPlansAppService : ApplicationService<ContractQualityPlansResource>, IContractQualityPlansAppService
    {
        ...
        public async Task<IDataResult<IList<ListContractQualityPlansDto>>> GetListAsync(ListContractQualityPlansParameterDto input)
        {
            var query = queryFactory
                   .Query()
                   .From(Tables.ContractQualityPlans)
                   .Select<ContractQualityPlans>(s => new { s.DocumentNumber, s.Id , s.ProductID})
                   .Join<Products>
                    (
        ...
        public async Task<int> RevisionNoControlAsync(Guid contractQualityControlPlanId, string revisionNo)
        {
            var query = queryFactory.Query().From(Tables.ContractOperationPictures).Count("Id").Where(new { ContractQualityPlanID = contractQualityControlPlanId, RevisionNo = revisionNo }, "");
