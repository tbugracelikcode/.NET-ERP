...
namespace TsiErp.Business.Entities.MaintenancePeriod.Services
{
    ...
    public class MaintenancePeriodsAppService : ApplicationService<MaintenancePeriodsResource>, IMaintenancePeriodsAppService
    {
        ...
        {
            var query = queryFactory.Query().From(Tables.MaintenancePeriods).Select<ListMaintenancePeriodsDto>(s => new { s.Code, s.Name, s.PeriodTime, s.Description_ }).Where(null, "");
        ...
        {
            var entityQuery = queryFactory.Query().From(Tables.MaintenancePeriods).Select("Id").Where(new { Id = id }, "");
