...
namespace TsiErp.Business.Entities.WorkOrder.Services
{
    ...
    public class WorkOrdersAppService : ApplicationService<WorkOrdersResource>, IWorkOrdersAppService
    {
        ...
        public async Task<IDataResult<SelectWorkOrdersDto>> GetbyProductionOrderIdAsync(Guid? id)
        {
            using (var connection = queryFactory.ConnectToDatabase())
            {
                var query = queryFactory
                        .Query().From(Tables.WorkOrders).Select("*")
                            ...
                            .Where(new { ProductionOrderID = id }, false, false, Tables.WorkOrders);

                var workOrder = queryFactory.Get<SelectWorkOrdersDto>(query);

                LogsAppService.InsertLogToDatabase(workOrder, workOrder, LoginedUserService.UserId, Tables.WorkOrders, LogType.Get, id.GetValueOrDefault());
