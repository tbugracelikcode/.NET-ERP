...
{
    public partial class AdminMachineOEEAnalysisPage
    {

        List<AdminMachineOEEChart> MachineOEEList = new List<AdminMachineOEEChart>();

        List<AdminMachineOEEGrid> MachineOEEGridList = new List<AdminMachineOEEGrid>();
        ...
        SfGrid<AdminMachineOEEGrid> Grid;
        ...
        protected override async void OnInitialized()
        {
            ...
            MachineOEEList = (await AdminDashboardAppService.GetAdminMachineChart(startDate, endDate));
            MachineOEEGridList = (await AdminDashboardAppService.GetAdminMachineGrid(startDate, endDate));

            if (MachineOEEList != null && MachineOEEList.Count > 0)
            {
            ...
            if (MachineOEEGridList != null && MachineOEEGridList.Count > 0)
            {
                foreach (var oee in MachineOEEGridList)
                {
                    oee.MONTH = L[oee.MONTH] + " " + oee.YEAR.ToString();
                }
            }
        ...
        public void CellInfoHandler(QueryCellInfoEventArgs<AdminMachineOEEGrid> Args)
        {
