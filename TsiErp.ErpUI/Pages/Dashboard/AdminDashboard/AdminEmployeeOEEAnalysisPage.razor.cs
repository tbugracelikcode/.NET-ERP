...
{
    public partial class AdminEmployeeOEEAnalysisPage
    {
        List<AdminEmployeeOEEChart> EmployeeOEEList = new List<AdminEmployeeOEEChart>();

        List<AdminEmployeeOEEGrid> EmployeeOEEGridList = new List<AdminEmployeeOEEGrid>();
        ...
        SfGrid<AdminEmployeeOEEGrid> Grid;
        ...
        protected override async void OnInitialized()
        {
            ...
            EmployeeOEEList = (await AdminDashboardAppService.GetAdminEmployeeChart(startDate, endDate));
            EmployeeOEEGridList = (await AdminDashboardAppService.GetAdminEmployeeGrid(startDate, endDate));

            if (EmployeeOEEList != null && EmployeeOEEList.Count > 0)
            {
                foreach (var oee in EmployeeOEEList)
                {
            ...
            if (EmployeeOEEGridList != null && EmployeeOEEGridList.Count > 0)
            {
                foreach (var oee in EmployeeOEEGridList)
                {
                    oee.MONTH = L[oee.MONTH] + " " + oee.YEAR.ToString();
                }
            }
        ...
        public void CellInfoHandler(QueryCellInfoEventArgs<AdminEmployeeOEEGrid> Args)
        {
