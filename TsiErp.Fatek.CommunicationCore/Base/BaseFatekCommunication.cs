...
namespace TsiErp.Fatek.CommunicationCore.Base
{
    public class BaseFatekCommunication
    {
        ...
        public string GetMessageWriteSingleDiscrete(int slaveStationNo, CommandCode cmmdCode, RunningCode runCode, MemoryType mType, ushort discreteNo, DataType dType)
        {
            ...
            {
                case DataType.BOOL:
                    frame += string.Format("{0}{1}", mType, discreteNo.ToString().PadLeft(4, '0')); // Discrete No.
                    break;
                default:
        ...
        public string GetMessageReadDiscretes(int slaveStationNo, CommandCode cmmdCode, ushort numberOfPoints, MemoryType mType, ushort startNo, DataType dType)
        {
            ...
            {
                case DataType.BOOL:
                    frame += string.Format("{0}{1}", mType, startNo.ToString().PadLeft(4, '0')); // Start No.
                    break;
                default:
                    throw new InvalidOperationException("Data type is invalid");
            }
        ...
        public string GetMessageReadRegisters(int slaveStationNo, CommandCode cmmdCode, ushort numberOfPoints, MemoryType mType, ushort startRegister, DataType dType)
        {
            ...
            switch (dType)
            {
                case DataType.INT:
                case DataType.WORD:
                    frame += string.Format("{0}{1}", mType, startRegister.ToString().PadLeft(5, '0')); // Start register No.
                    break;
                case DataType.DINT:
                case DataType.DWORD:
                case DataType.REAL:
                    frame += string.Format("{0}{1}", mType, startRegister.ToString().PadLeft(8, '0')); // Start register No.
                    break;
                default:
                    throw new InvalidOperationException("Data type is invalid");
            }
        ...
        /// <returns>string</returns>
        public string GetMessageWriteMultipeRegisters(int slaveStationNo, CommandCode cmmdCode, ushort numberOfPoints, MemoryType mType, ushort startRegister, DataType dType, int data)
        {
            ...
            switch (dType)
            {
                ...
                //    break;
                case DataType.WORD:
                    //if(mType != MemoryType.D)
                    //{
                    //    frame += string.Format("{0}{1}", mType, startRegister.ToString().PadLeft(5, '0')); 
                    //    values += data.ToString("X").PadLeft(8, '0');
                    //}
                    //else
                    //{
                    //    frame += string.Format("{0}{1}", mType, startRegister.ToString().PadLeft(5, '0')); 
                    //    values += data.ToString("X").PadLeft(4, '0');
                    //}
